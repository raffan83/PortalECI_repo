	...
	init: function( editor ) {
		editor.on( 'instanceReady', function() {
			...
			function dataCallback( matchInfo, callback ) {
				...
				callback( suggestions.map(function(placeholder){
					return {id:placeholder,  output:"${"+placeholder+"}"};
				}) );
			}
			...
			config.itemTemplate = '<li data-id="{id}">{output}</li>';
			config.outputTemplate = '{output}';
